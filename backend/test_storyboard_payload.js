...
async function test(body) {
  try {
    ...
    console.log('Body:', JSON.stringify(body), '-> Status:', response.status, 'Data:', response.data);
  } catch (error) {
    if (error.response) {
      console.log('Body:', JSON.stringify(body), '-> Error Status:', error.response.status, 'Data:', error.response.data);
    } else {
      console.log('Body:', JSON.stringify(body), '-> Error:', error.message);
    }
  }
}
...
async function run() {
  ...
  // Test Format 4 (Sami payload structure)
  await test({
    tasks: [{
      text: "Xin chào bạn",
      voice: "BV421_vivn_streaming"
