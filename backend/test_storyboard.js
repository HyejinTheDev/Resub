...
async function testEndpoint(url) {
  try {
    ...
    console.log(`URL: ${url} -> Status: ${response.status}, Data:`, response.data);
  } catch (error) {
    if (error.response) {
      console.log(`URL: ${url} -> Error Status: ${error.response.status}, Data:`, error.response.data);
    } else {
...
async function run() {
  await testEndpoint('https://edit-api-sg.capcut.com/storyboard/v1/tts/multi_platform');
  await testEndpoint('https://edit-api-sg.capcut.com/lv/v1/storyboard/v1/tts/multi_platform');
}
