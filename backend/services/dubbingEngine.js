      ...
      activeMasks.forEach(mask => {
        ...
        if (fastBlur) {
          ...
          filterGraph += `;[${currentVInput}]split[${mainLabel}][${cropLabel}];` +
                         ...
                         `drawbox=x=0:y=0:w=iw:h=ih:color=${ffmpegColor}@${coverOpacity}:t=fill[${blurredSrcLabel}];` +
                         `[${toMaskLabel}]scale=w=${maskWpx}:h=${maskHpx},drawbox=x=0:y=0:w=iw:h=ih:color=black:t=fill,drawbox=x=iw*0.01:y=0:w=iw*0.98:h=ih:color=white:t=fill,boxblur=luma_radius=${Math.max(1, Math.min(30, Math.floor((maskWpx - 1) / 2), Math.floor((maskHpx - 1) / 2)))}:luma_power=3:chroma_radius=0:chroma_power=0[${alphaMaskLabel}];` +
